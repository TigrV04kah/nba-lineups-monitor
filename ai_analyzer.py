...
ПОСЛЕДНИЕ 10 ИГР КОМАНДЫ (от самой последней к более ранним - игра #1 это ПОСЛЕДНЯЯ):"""
    ...
    # Добавляем информацию о травмах команды из RotoWire
    injuries_text = ""
    if team_injuries:
        ...
        for injured_player in team_injuries:
            injuries_text += f"\n• {injured_player} - OUT (НЕ ИГРАЕТ!)"
        injuries_text += f"\n\n→ Это РЕАЛЬНЫЕ данные о составе НА СЕГОДНЯ. Эти игроки ОТСУТСТВУЮТ."
        injuries_text += f"\n→ {player_name} получит увеличенную нагрузку из-за отсутствия этих игроков."
...
3. **Разделение ФОРМЫ и РОЛИ**:
   ...
   - ⚠️ ПРАВИЛО: Если тренд очков нестабильный, НО роль увеличилась (OUT партнёры, рост минут) → ПРИОРИТЕТ РОЛИ над формой

4. **Анализ влияния СОСТАВОВ команды**:
   - Посмотри раздел "СОСТАВЫ КОМАНДЫ В ПОСЛЕДНИХ 10 ИГРАХ"
   - С какими партнёрами игрок показывал лучшую/худшую статистику?
   - ⚠️ ТРАВМИРОВАННЫЕ игроки (раздел выше) НЕ БУДУТ играть сегодня - исключи их из анализа текущего состава

5. **Анализ силы соперника**:
...
⚠️ КРИТИЧЕСКИ ВАЖНО:
- ЕСЛИ указано "Полный состав, нет травмированных" → НЕ ПИШИ о возможных травмах
- ЕСЛИ указан список OUT игроков → учитывай их ОБЯЗАТЕЛЬНО
...
**3. Роль в команде**: [primary scorer / secondary option / role player]
   → Изменение роли из-за травм: [да/нет, как влияет]

**4. Соперник**: [сильный/средний/слабый], допускает [X] очков на позицию

**5. ПРОГНОЗ**:
   - База (среднее 3 игр): [X] очков
   - Корректировки: [перечислить]
   - **Очки: X-Y** | Подборы: A-B | Передачи: C-D
   - Роль: [primary / secondary / role player]
...
2. МИНУТЫ - главный драйвер статистики.
3. РОЛЬ важнее формы при нестабильном тренде.
