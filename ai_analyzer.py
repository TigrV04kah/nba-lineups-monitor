    ...
    if team_games:
        ...
        for i, game in enumerate(team_games[:5], 1):
            ...
            if starters:
                ...
                # Показываем производительность игрока в этом составе
                for starter in starters:
                    if starter.get('name') == player_name:
                        lineups_context += f"\n    → {player_name}: {starter.get('pts', 0)}pts, {starter.get('reb', 0)}reb, {starter.get('ast', 0)}ast"
                        break
...
- Базируй прогноз на: (1) ПОЛНОЙ динамике формы по всем 5 играм, (2) силе соперника, (3) РЕАЛЬНОМ составе команды

📊 ФОРМАТ ОТВЕТА:
**1. Тренд формы (обязательно укажи цифры из каждой игры)**:
   - Игра №5: X очков
   - Игра №4: Y очков
   - Игра №3: Z очков
   - Игра №2: W очков
   - Игра №1: V очков
   → Тренд: [восходящий/нисходящий/стабильный/волатильный]
