...
ПОСЛЕДНИЕ 5 ИГР (от самой последней к более ранним - игра #1 это ПОСЛЕДНЯЯ сыгранная):"""
    ...
    if team_injuries:
        ...
        for injured_player in team_injuries:
            injuries_text += f"\n• {injured_player} - OUT"
        injuries_text += f"\n\n→ Это РЕАЛЬНЫЕ данные о составе. {player_name} получит увеличенную нагрузку из-за отсутствия этих игроков."
    else:
...
2. **Анализ влияния СОСТАВОВ команды** (КРИТИЧЕСКИ ВАЖНО):
   - Посмотри раздел "СОСТАВЫ КОМАНДЫ В ПОСЛЕДНИХ 5 ИГРАХ" выше
   - Обрати внимание: с какими партнёрами игрок показывал лучшую/худшую статистику
   - Если в последних играх состав менялся → найди закономерность:
     * С каким составом у игрока были больше очков?
     * Когда у него была лучшая производительность?
   - Сравни прошлые составы с ТЕКУЩИМ составом (учитывая травмы из раздела "ТРАВМИРОВАННЫЕ/ВЫБЫВШИЕ")
   - Если состав сегодня похож на тот, где игрок показывал высокие очки → увеличь прогноз
   - Если состав сегодня другой (из-за травм) → оцени как это повлияет на роль игрока
