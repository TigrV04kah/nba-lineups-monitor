...
1. **Анализ тренда ОЧКОВ (10 игр)**:
   - Запиши очки: Игра №10 → ... → №2 → №1
   - ⚠️ АНОМАЛИИ: Если одна игра отклоняется от среднего >40% → пометь её [АНОМАЛИЯ] и НЕ делай на ней вывод о тренде
   - Определи тренд: ВОСХОДЯЩИЙ / НИСХОДЯЩИЙ / СТАБИЛЬНЫЙ (±3 очка) / ВОЛАТИЛЬНЫЙ

2. **Анализ тренда МИНУТ (ключевой фактор!)**:
   - Запиши минуты: Игра №10 → ... → №2 → №1
   - Определи тип тренда:
...
**1. Тренд очков**: №10→...→№2→№1: [цифры, пропуски отмечай X]
   → Тренд: [тип], аномалии: [есть/нет], пропуски: [количество]

**2. Тренд минут**: №10→...→№2→№1: [цифры, пропуски отмечай X]
...
Ключевые принципы:
1. МИНУТЫ - главный драйвер статистики. Всегда анализируй связь минут и очков.
2. РОЛЬ важнее формы при нестабильном тренде. Если роль растёт (OUT партнёры, больше минут) - это перевешивает плохую форму.
